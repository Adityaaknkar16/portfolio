...
function SectionHeading({ label, title, direction = 'left' }) {
  return (
    <ScrollReveal direction={direction}>
      ...
      <h2 className="mt-2 font-display text-4xl italic text-cream md:text-5xl">{title}</h2>
    </ScrollReveal>
  );
}
...
export default function About() {
  return (
    <section id="about" className="section-surface-alt relative py-24 md:py-32">
      <SectionMarginFX tag="// about" />
        ...
        <div className="mt-16 grid gap-12 lg:grid-cols-2 lg:gap-16">
          <div className="space-y-8">
            <ScrollReveal direction="left" delay={0.1}>
              <p className="text-lg leading-relaxed text-cream/75">{profile.summary}</p>
            </ScrollReveal>

            <ScrollReveal direction="depth" delay={0.2}>
              <div className="grid grid-cols-2 gap-4">
                {stats.map((stat) => (
                  <div key={stat.label} className="stat-accent glass rounded-lg p-4">
                    <p className="font-display text-3xl text-gold">{stat.value}</p>
                    <p className="mt-1 font-mono text-xs text-cream/50">{stat.label}</p>
                  </div>
                ))}
              </div>
            </ScrollReveal>
          ...
          <ScrollReveal direction="right" delay={0.15}>
            <div className="glass rounded-xl border border-white/5 p-6 md:p-8">
