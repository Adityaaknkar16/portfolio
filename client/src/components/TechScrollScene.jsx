    ...
    <motion.svg className={className} style={style} viewBox="0 0 200 800" fill="none" aria-hidden>
      ...
      {[40, 160, 320, 480, 640, 720].map((y, i) => (
        <motion.circle
          ...
          r="3"
          fill="#00ff66"
          animate={{ opacity: [0.15, 0.85, 0.15], scale: [1, 1.2, 1] }}
...
function TerminalPanel({ title = 'dev-environment' }) {
  return (
    <div className="tech-terminal w-[min(240px,18vw)]">
      <motion.div className="mb-2 flex gap-1.5" animate={{ opacity: [0.6, 1, 0.6] }} transition={{ duration: 2, repeat: Infinity }}>
        <span className="h-2 w-2 border border-border bg-red-950" />
        <span className="h-2 w-2 border border-border bg-accent/30" />
        <span className="h-2 w-2 border border-border bg-accent" />
      </motion.div>
      <p className="font-mono text-[9px] text-zinc-500">{title}</p>
      <p className="mt-2 font-mono text-[11px] text-accent">$ whoami</p>
      <p className="font-mono text-[11px] text-zinc-300">aditya — full_stack_dev</p>
      <p className="mt-2 font-mono text-[11px] text-accent">$ stack --list</p>
      <p className="font-mono text-[11px] text-zinc-100">MERN · GenAI · RAG</p>
      <motion.p
        className="mt-2 font-mono text-[10px] text-accent/60"
    ...
    <div className="server-rack">
      {[0, 1, 2, 3].map((i) => (
        ...
        >
          ...
          <span className="font-mono text-[8px] text-accent/60">srv_0{i + 1}</span>
        </motion.div>
      ))}
    </div>
  );
...
function AINodeCluster() {
  return (
    <div className="ai-cluster">
      <motion.div className="ai-core font-bold" animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 3, repeat: Infinity }}>
        <span className="font-mono text-xs text-accent">RAG</span>
      </motion.div>
...
function ZoneVisual({ zone, scrollRotate }) {
  ...
  if (zone.type === 'laptop') {
    return (
      ...
      >
        <Laptop3D />
        <div className="absolute -inset-8 -z-10 bg-accent/5 blur-3xl" />
      </motion.div>
    );
  }
...
export default function TechScrollScene() {
  ...
  return (
    ...
    >
      {/* Layer 0 */}
      <motion.div className="absolute inset-0 grid-hero" style={{ y: gridY, opacity: gridOpacity }} />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_20%_30%,rgba(0,255,102,0.02),transparent_50%)]" />
      <motion.div
        className="absolute inset-0 bg-[radial-gradient(ellipse_at_85%_70%,rgba(255,255,255,0.01),transparent_45%)]"
        style={{ y: layer1Y }}
      />
      ...
      <motion.div className="absolute inset-x-0 top-0 h-[420vh]" style={{ y: sceneY }}>
        ...
        <motion.div className="absolute inset-0" style={{ y: layer4Y }}>
          {[...Array(20)].map((_, i) => (
            <motion.span
              key={i}
              className="absolute font-mono text-[10px] font-medium text-accent/20"
              style={{
            ...
            >
              {i % 3 === 0 ? '0x00FF66' : i % 3 === 1 ? '{ }' : '01'}
            </motion.span>
          ))}

          <motion.div
            className="absolute left-[6%] top-0 h-full w-px bg-gradient-to-b from-transparent via-accent/20 to-transparent"
            animate={{ opacity: [0.3, 0.7, 0.3] }}
            transition={{ duration: 4, repeat: Infinity }}
          />
          <motion.div
            className="absolute right-[28%] top-0 h-full w-px bg-gradient-to-b from-transparent via-accent/20 to-transparent"
          ...
          {/* Scan lines */}
          <motion.div
            className="absolute inset-x-0 h-px bg-gradient-to-r from-transparent via-accent/15 to-transparent"
            animate={{ top: ['0%', '100%'] }}
        ...
        {/* Corner rings */}
        <motion.div
          className="absolute left-[1%] top-[8%] h-24 w-24 border border-accent/10"
          animate={{ rotate: 360, scale: [1, 1.05, 1] }}
          transition={{ rotate: { duration: 20, repeat: Infinity, ease: 'linear' }, scale: { duration: 4, repeat: Infinity } }}
        />
        <motion.div
          className="absolute right-[1%] bottom-[12%] h-32 w-32 border border-dashed border-accent/15"
          animate={{ rotate: -360 }}
      ...
      {/* Vignette */}
      <motion.div className="absolute inset-0 bg-gradient-to-r from-bg/30 via-transparent to-bg/30" />
